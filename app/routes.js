...
router.post('/add-job-capability', [
	...
	check('jobCapability', 'Job Capability must only contain letters and space').matches('^[\.a-zA-Z0-9,!? ]*$')
], async (req, res) => {
	// variable to store error details
	errors = validationResult(req);
...
router.post('/add-job-role', [
	...
	check('jobTitle', 'Job Title: Must only contain letters and space').matches('^[\.a-zA-Z0-9,!? ]*$'),
